...
class PayPalPayment {
	...
	public function generateAccessToken() {
		...
		if ($this->sandbox_mode) {
			curl_setopt($ch, CURLOPT_URL, "https://api-m.sandbox.paypal.com/v1/oauth2/token");  //DUMMY
		} else {
	...
	public function executePayment($paymentID, $payerID) {
		if ($this->sandbox_mode) {
			$paypal_url = "https://api-m.sandbox.paypal.com/v2/checkout/orders/".$paymentID."/capture/";
		} else {
			$paypal_url = "https://api-m.paypal.com/v2/checkout/orders/".$paymentID."/capture/";
		}
		$authorization = "Authorization: Bearer ".$this->getAccessToken();

		$data = ["payer_id" => $payerID, "id" => $paymentID];
